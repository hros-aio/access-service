...
```bash
curl -X GET "http://localhost:3000/user-groups/UUID_HERE/scope" \
...
```bash
curl -X POST "http://localhost:3000/user-groups/UUID_HERE/scope/impact-estimate" \
  -H "Authorization: Bearer ADMIN_JWT_TOKEN" \
  -H "X-Tenant-Code: tenant-default" \
...
```bash
curl -X PUT "http://localhost:3000/user-groups/UUID_HERE/scope" \
  -H "Authorization: Bearer ADMIN_JWT_TOKEN" \
  -H "X-Tenant-Code: tenant-default" \
  -H "Content-Type: application/json" \
  -d '{
