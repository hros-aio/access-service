...
2. Send `POST /api/v1/auth/login` request with header/context `sourceIp: "192.168.1.50"`.
3. **Expected Outcome**: HTTP 200 / Login succeeds.

### Scenario 2: Unapproved IP Address Access
1. Configure tenant `TENANT_01` with `ip_restriction_enabled: true` and `allowed_ip_cidrs: ["192.168.1.0/24"]`.
2. Send `POST /api/v1/auth/login` request with header/context `sourceIp: "10.0.0.1"`.
...
1. Send `POST /api/v1/auth/password/reset-request` from unapproved IP `10.0.0.1`.
