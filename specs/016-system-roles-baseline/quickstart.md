...
1. Send `DELETE /api/v1/roles/:employeeRoleId` with valid admin credentials.
...
1. Send `PUT /api/v1/roles/:adminRoleId/permissions` omitting `role.manage` (a protected capability).
...
1. Send `PUT /api/v1/roles/:employeeRoleId/permissions` including all protected capabilities plus `location.view`.
...
1. Send `PATCH /api/v1/roles/:employeeRoleId/rename` with `{ "name": "Team Member" }`.
